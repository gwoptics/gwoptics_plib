 ...
 * The class accepts user given values for nodes which are used to build a lookup table for
 * fast access of values. The lookup table ranges from 0.0f to 1.0f where the steps in between are changeable via the
 * constructor. To lookup a colour value the user input of the location must normalise their
 * input and select a value between 0.0 and 1.0(Values greater or less than these are capped). 
 * So for a range of 0.0 to 100.0 to get the colour of a point whose value is 69, the location
 * input should be 0.69f.
 * </p>
 * 
 * <p>
 * This class implements the general IColourmap interface which allows it to work with objects
 * that use colourmaps.
 * </p>
 * 
...
public class RGBColourmap implements IColourmap {
	...
	public boolean isCentreAtZero(){return _isCentreAtZero;}
	public void setCentreAtZero(boolean value){_isCentreAtZero = value;}
	...
	/**
	 * This function adds an RGBNode to the colour map, which is then used to generate 
	 * various gradients of colour.
	 **/
	 ...
	 * 
	 * Once the required nodes have been added to the colourmap using addNode(), this function
	 * iterates through them all to generate the colourmap lookup table. This can later be accessed 
	 * via the getColourAtLocation() and getIntAtLocation() functions. This function populates both
	...
	public ColourmapNode getNode(int i) {
		if(_nodes.size() <= i){throw new ArrayIndexOutOfBoundsException();}
		return _nodes.get(i);
	}
