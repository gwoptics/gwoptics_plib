...
/**
 * <p>
 * Axis2D is a straight forward object for drawing 2D axes. It has all the
 * functionality for tick marks, minimum and maximum values, colour and size,
 * tick labels and axis labels.
 * </p>
 * 
 * <p>
 * Once an axis object has been created it is down the the user to specify the
 * necessary rotations of labels to make it readable. All the functions
 * beginning with set should be used to alter the axis.
 * </p>
 * 
...
public class Axis2D extends Renderable implements PConstants {
	 ...
	 * 
	 * @param lblpos
	 *            position can be START, MIDDLE, END or OUTSIDE
	 */
	 ...
	 * rendered. This is used in situations with vertical axes and the tick
	 * labels vary in length as the values get larger.
	 */
	...
	/**
	 * Sets an offset that is to be applied to the major tick labels. This
	 * offset is applied in the direction of the ticks away from the axis.
	 */
	 ...
	 * logarithmic axis the value you pass in here is taken to mean
	 * 10^(spacing). Hence setting a spacing of 1.0f means the ticks are
	 * separated by factors of 10, spacing 2.0f gives a spacing of factors of
	 * 100 etc.
	 */
	...
	/**
	 * Sets the number of decimal places to display in each tick label. Only
	 * valid for tick valueTypes of DECIAML or EXPONENT
	 */
	...
	/**
	 * Axis2D constructor accepting the parent PApplet and the length of the
	 * axis in pixels
	 */
	...
	protected void _generateTicks() {
		...
		if (_isLogarithmic) {
			...
			firstTickOffset = (Math.ceil(spcDivMn) * _majorTickSpacing - Math
					.log10(_minShow));
		...
		while (currTickValue <= max) {
			...
			while (currMinorTickValue <= max
				...
				if (_isLogarithmic)
					...
					_minorTickPositions
							.add(valueToPosition(currMinorTickValue));
	...
	protected void _drawAxisLabel() {
		...
		_parent.text(String.valueOf(_label), 0, 0.25f * _font.getFont()
				.getSize(), 0);
		_parent.popMatrix();
	}
}
