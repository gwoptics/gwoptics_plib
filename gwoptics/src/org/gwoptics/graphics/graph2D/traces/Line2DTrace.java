...
/**
 * <p>
 * The Line2DTrace is the default implementation of the IGraphTrace interface. It can be used for a variety
 * of plotting tasks and for 9/10 times will be useful for whatever is needed. 
 * </p>
 * 
 * <p>
 * The Line2DTrace object requires that an IGraph2DCallback object is provided for it to plot a trace. To plot
 * the trace, the generate() method is first called. This populates an internal array that stores the points of 
 * the trace. If the callback object changes, the trace will not change until the generate() method is called
 * again. If you have a trace that changes regularly, ie a non-static standing wave, then call the generate
 * method in each draw() call. <b>Note:</b> Make sure the callback object is optimised as much as possible for non
 * static graphs, complicated graphs that are regenerated each draw call will grind the system to a halt, contemplate
 * using the TrigLookup object for faster sinusoidal functions.
 * </p>
 * 
 * <p>
 * This trace object also accepts trace effects defined by the ITraceColourEffect interface. Use this to visually 
 * enhance a graph trace.
 * </p>
 * 
 * <p>
 * <b>Custom 2D Traces</b><br>
 * If the need arises to create a custom trace object for the graph2D control two options are open. Implement the IGraphTrace
 * interface from scratch or inherit the Line2DTrace object, the later being the simplest. To do that though understanding of
 * the class is necessary and its members.
 * </p>
 * <p>
 * Most of the public members are properties such as setAxis and setTraceColour, these simple apply a value to the relevant member
 * and most likely will not need overriding. The most likely candidates for overriding are the generate and draw methods. Generate
 * simply goes about deducing the graph value for each pixel on the x-axis and then passing this to the ILineEquation object to
 * produce a y value. This is converted into a point that is represented on the Y-axis and then stored in the _pointData and _eqData
 * arrays. The generate function therefore only needs to be called on if the trace is changing values or shape. 
 * </p>
 * <p>
 * The draw method is the one who goes about plotting the values using the _pointData array. No heavy calculations should be going
 * on in the draw method, to give the best framerates. Draw loops through each _pointData value and limits it to the max and min 
 * values of the y-axis and then draws the points using connected lines. At this point any trace colour effect is also applied to the
 * trace. One important note is that if you want to display no point at a place on your trace, simply put Float.NaN into the _pointData
 * array. The draw method then ignores this point.
 * </p>
 * <p>
 * The recommended approach to producing a new trace is to override the generate method and populate the relevant arrays as needed.
 * Then let the draw method do its job, this is recommended if what you are looking for in the end is a line. If for example an area
 * graph, bar-graph, histogram or scatter plot is required then the draw method would be need to be altered to produce the correct
 * effect. For future use if no source is present the generate and draw method code is included below, all the other methods can be
 * left not overridden.
 * </p>
 * <pre>
 * <code>
