 ...
 * ,though it is more common to use <1,0,0>, <0,1,0> or <0,0,1>, using setAxesDirection(). The axis has full tick functionality
 * using both major and minor ticks, with labels for values along the axis shown for each major tick. As the axis is 3D you
 * must specify which plane the labels and ticks must be drawn using setLabelDirection(). Unless strange looking axes are required
 * it is recommended to use a label direction which is orthogonal to the axis direction. As Axis3D extends Renderable it has a position
 * member which can be used to position where the axis is. Both the axis label and tick labels can be rotated using the given functions
 * to generate the required layout of the axis. 
 * 
 * History
 * Version 0.3.8 Added billboarding to the tick labels, must be used with Camera3D class though.
 * 
...
public class Axis3D extends Renderable implements PConstants {
	...
	public void setTickLabelBillboarding(boolean value){
		if(value){
			if(Camera3D.getLookat() == null)
				throw new RuntimeException("To use text billboards you must use the Camera3" +
	...
	public void setTickLabelXRotation(float val) {_axisTickLblRotation.x = val;}
	public void setTickLabelYRotation(float val) {_axisTickLblRotation.y = val;}
	public void setTickLabelZRotation(float val) {_axisTickLblRotation.z = val;}
	public void setLabelXRotation(float val) {_axisLblRotation.x = val;}
	public void setLabelYRotation(float val) {_axisLblRotation.y = val;}
	public void setLabelZRotation(float val) {_axisLblRotation.z = val;}
	...
	public void setMajorTicks(int t) {_majorTicks = t;}
	public void setMinorTicks(int t) {_minorTicks = t;}
	...
	public void setMaxValue(float val) {_maxShow = val;}
	public void setMinValue(float val) {_minShow = val;}	
	public void setLength(float l) {_length = l;}
	...
	public void draw() {
		...
		if(_drawTicks || _drawTickLabels){				
			...
			PVector dvTick = PVector.div(length, _majorTicks); // distance between major ticks
			PVector dvTickMinor = PVector.div(dvTick, _minorTicks + 1); // distance between minor ticks
			...
			PVector tickLblPos;
			String tickLbl = null;
			float dValue = (float) ((_maxShow - _minShow) / (_majorTicks)); //difference in label text between major ticks
			...
			for (int i = 0; i <= _majorTicks; i++) {
				...
				switch (_tickLblType) {
				case DECIMAL:			
					tickLbl = String.format("%." + _accuracy + "f",(float)(_minShow + i * dValue));		
					break;
				case EXPONENT:	
					tickLbl = String.format("%." + _accuracy + "E",(float)(_minShow + i * dValue));				
					break;
				case INTEGER:	
					tickLbl = String.format("%d",Math.round(_minShow + i * dValue));				
					break;
				}
				...
				//tick size
				tickLblPos = PVector.add(tickPos, PVector.mult(_labelDirection, 1 + _parent.textWidth(tickLbl)/2 + Math.abs(_majorTickSize)));
				//as the text is not vertically centred about the render point we offset slightly
				//depending on the size of the font so label is approx centred to tick
				tickLblPos.add(PVector.mult(_unitVec, (float) ((float) _axisTickLblSize * 1.2)));
				...
				if(_drawTicks){
					...
					// Draw minor ticks
					if(i != 0) {//dont draw minor tick if first iteration as the minor ticks get drawn in the -ive axis
								//direction from the major tick in question, was that or miss the last one out if drawn
								//in +ve axis direction.
						tickPosMinor = tickPos.get();
			
						for (int j = 0; j < _minorTicks; j++) {
							//same as with major ticks, get start and end points
							tickPosMinor = PVector.sub(tickPosMinor, dvTickMinor);
							tickEndMinor = PVector.add(tickPosMinor, PVector.mult(_labelDirection, _minorTickSize));
			
							_parent.beginShape(PConstants.LINE);
							_parent.vertex(tickPosMinor.x, tickPosMinor.y, tickPosMinor.z);
							_parent.vertex(tickEndMinor.x, tickEndMinor.y, tickEndMinor.z);
							_parent.endShape();
						}
					}
				}
		...
		if(_drawName){
			...
			lblPos.add(PVector.mult(_labelDirection, longestLabel + _axisLblSize + _axisLblOffset));
