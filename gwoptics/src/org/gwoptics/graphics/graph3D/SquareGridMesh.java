...
/**
 * <p>
 * SquareGridMesh is a square shaped grid with variable resolution along both sides. Each point can have its height set
 * allowing surfaces to be generated. This object is originally designed to work with the SurfaceGraph3D control, but is 
 * Usable without this for other purposes. 
 * </p>
 * 
 * <p>
 * Each point is coloured according to a supplied IColourmap, solid fill colour or by wireframe. The colour and point positions
 * are both stored in 2 arrays. Default values are a resolution along both sides of 10 squares each 10 units in length.
 * </p>
 * 
 * <p>
 * <b>Important Note:</b> It must be noted that although due to notation the Z axis points upwards, in world space the Y axis points
 * up. Therefore Z <-> Y when it comes to plotting data. 
 * </p>
 * 
...
public class SquareGridMesh extends Renderable {
	 ...
	 * wireframe mode with stroke colour as white.
	 * 
	...
	public SquareGridMesh(int X, int Y, float dx, float dz,PApplet parent) {
		...
		if(X <= 0 || Y <= 0) {throw new IllegalArgumentException("Grid size dimensions should be greater than 0.");}
	 ...
	 */
	public float getZValue(int X, int Y) {return _vertexs[X][Y][1];}
