 ...
 * It is possible to draw several surface traces, but care must be taken not to incorporate to many points if the 
 * graph is to be rendered with a moving camera, otherwise slower system will struggle to render the graph.
 * </p>
 * <p>
 * Once the graph is created, it is possible to add traces using the addSurfaceTrace() function. You must specify 
 * a callback for the trace which defines the equation it represents. The callback must implement the IGraph3DCallback
 * interface, which allows it to work with the SurfaceTrace3D class by providing a function to compute the values of each
 * point on the surface. It is also possible to specify a colourmap 
 * to colour the trace. Several presets can be found in org.gwoptics.graphics.colourmap.presets, or unique ones 
 * can be created using the RGBColourmap object. Once the trace is added it must be plotted everytime it is changed
 * using plotSurfaceTrace() otherwise changes will not be seen.
 * </p>
 * <p>
 * The graphs position denotes the point where all 3 axes meet, the x,y and z axes then all extend in the positive direction
 * by the length values supplied. If in the constructor setAxisCrossAtZero is true, position denotes the location of the 
 * minimum point of the z-axis. Axis x and y will then alter themselves automatically to get as close to zero on the z-axis
 * as possible. Due to screen space defining the y axis as 'down' the screen from the top corner, it is 
 * recommended that PApplet.scale(1,-1,1) is applied before drawing the graph or a suitable rotation. Or use the Camera3D 
 * class which defines the upwards direction to be -y automatically.
 * </p>
 * <p>
 * <b>Important Note:</b> Due to notation the x and y directions should relate to the grid plane the the height in the z
 * direction. Though due to the way screen space works y is the up direction. This must be taken into account throughout 
 * this class.
 * </p>
 * 
 * <p>
 ...
 * <br/><br/>
 * Version 0.2.4 sees a breaking change of notation relating z as the up direction, see important note above for more implications
 * of this change. Also changed class to final, so it cant be extended.
 * <br/><br/>
...
public final class SurfaceGraph3D extends Renderable {
	...
	public float getZAxisMax(){return _az.getMaxValue();}
	
	//Setters
	public void setXAxisMin(float l){_ax.setMinValue(l);_alterSurfaceSpacing();_alignAxesToZeroPoint();}
	public void setYAxisMin(float l){_ay.setMinValue(l);_alterSurfaceSpacing();_alignAxesToZeroPoint();}
	public void setZAxisMin(float l){_az.setMinValue(l);_alignAxesToZeroPoint();}
	public void setXAxisMax(float l){_ax.setMaxValue(l);_alignAxesToZeroPoint();}
	public void setYAxisMax(float l){_ay.setMaxValue(l);_alterSurfaceSpacing();_alignAxesToZeroPoint();}
	public void setZAxisMax(float l){_az.setMaxValue(l);_alterSurfaceSpacing();_alignAxesToZeroPoint();}
	
	public void setXAxisLabel(String s) {_ax.setAxisLabel(s);}
	public void setYAxisLabel(String s) {_ay.setAxisLabel(s);}
	public void setZAxisLabel(String s) {_az.setAxisLabel(s);}
	
	public void setXAxisLabelAccuracy(int l){_ax.setTickLabelAccuracy(l);}
	public void setYAxisLabelAccuracy(int l){_ay.setTickLabelAccuracy(l);}
	public void setZAxisLabelAccuracy(int l){_az.setTickLabelAccuracy(l);}
	
	public void setXAxisLabelType(ValueType l){_ax.setTickLabelType(l);}
	public void setYAxisLabelType(ValueType l){_ay.setTickLabelType(l);}
	public void setZAxisLabelType(ValueType l){_az.setTickLabelType(l);}
	
	public void setXAxisMajorTicks(int n) {_ax.setMajorTicks(n);}
	public void setXAxisMinorTicks(int n) {_ax.setMinorTicks(n);}
	public void setYAxisMajorTicks(int n) {_ay.setMajorTicks(n);}
	...
	/**
	 * Use this to set a surface to automatically set the min and max values
	 * of the Z-axis to match the computed values, resulting in no clipping of
	 * surface points.
	 * 
	...
	/**
	 * All that needs to be specified to generate a graph is its dimensions. This sets a default state for all the axes,
	 * like which direction the labels are drawn in and label sizes etc. Some of these properties are changeable using the 
	 * various axis setter functions.
	 * 
	 ...
	 * @param yLength Length of the Y-Axis
	 * @param zLength Length of the Z-Axis
	 * @param setAxisCrossAtZero boolean stating whether to make axes cross at zero
	 */
	...
	private void _alignAxesToZeroPoint(){
		if(_setAxisCrossAtZero && (_az.getMinValue() <= 0 && _az.getMaxValue() >= 0)){			
			float pos;
			
			pos = Math.abs(_az.getLength() * Math.abs(_az.getMinValue()/(_az.getMaxValue() - _az.getMinValue())));
	 ...
	 * @param XRes Number of squares along x side of grid, the higher the more detailed the graph
	 * @param YRes Number of squares along z side of grid, the higher the more detailed the graph
	 * @param map Null for wireframe rendering. Otherwise specifies how to colour the surface.
	 */
	public void addSurfaceTrace(IGraph3DCallback cb, int XRes, int YRes, IColourmap map){
		synchronized(_traces){
			_traces.add(new SurfaceTrace3D(_parent,_ax,_ay,_az,XRes,YRes,cb,String.valueOf(_traces.size() + 1),map));
		}
	}
	...
	public synchronized void draw() {
		synchronized(_traces){
			...
			//Although it looks better if it is slightly offset in wireframe mode
			//it appears that you gain an extra fps(on my computer) if they arent 
			//applied.
