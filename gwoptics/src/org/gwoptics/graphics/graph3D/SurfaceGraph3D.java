...
/**
 * <p>
 * This class incorporates the Axis3D and SurfaceTrace3D objects to construct a
 * 3D Cartesian surface graph. It is possible to draw several surface traces,
 * but care must be taken not to incorporate to many points if the graph is to
 * be rendered with a moving camera, otherwise slower system will struggle to
 * render the graph.
 * </p>
 * <p>
 * Once the graph is created, it is possible to add traces using the
 * addSurfaceTrace() function. You must specify a callback for the trace which
 * defines the equation it represents. The callback must implement the
 * IGraph3DCallback interface, which allows it to work with the SurfaceTrace3D
 * class by providing a function to compute the values of each point on the
 * surface. It is also possible to specify a colourmap to colour the trace.
 * Several presets can be found in org.gwoptics.graphics.colourmap.presets, or
 * unique ones can be created using the RGBColourmap object. Once the trace is
 * added it must be plotted everytime it is changed using plotSurfaceTrace()
 * otherwise changes will not be seen.
 * </p>
 * <p>
 * The graphs position denotes the point where all 3 axes meet, the x,y and z
 * axes then all extend in the positive direction by the length values supplied.
 * If in the constructor setAxisCrossAtZero is true, position denotes the
 * location of the minimum point of the z-axis. Axis x and y will then alter
 ...
 * defines the upwards direction to be -y automatically.
 * </p>
 * <p>
 * <b>Important Note:</b> Due to notation the x and y directions should relate
 * to the grid plane the the height in the z direction. Though due to the way
 * screen space works y is the up direction. This must be taken into account
 ...
 * Can now alter colour of tick and fonts of axis <br/>
 * <br/>
 * Version 0.3.0 Altered to ensure labels were correctly aligned and that the
 * graph was right-handed. <br/>
 * <br/>
 ...
 * Also changed class to final, so it cant be extended. <br/>
 * <br/>
...
public final class SurfaceGraph3D extends Renderable {
	 ...
	 * 
	 * @param surfaceIndex
	 *            The index of the surface to autorange the z axis
	 */
	 ...
	 * This sets a default state for all the axes, like which direction the
	 * labels are drawn in and label sizes etc. Some of these properties are
	 * changeable using the various axis setter functions.
	 * 
	 * @param p
	 *            PApplet that will render this object
	 * @param xLength
	 ...
	 * @param zLength
	 *            Length of the Z-Axis
	 * @param setAxisCrossAtZero
	 *            boolean stating whether to make axes cross at zero
	 */
	 ...
	 * 
	 * @param cb
	 *            IGraph3DCallback that represents the equation of the trace
	 * @param XRes
