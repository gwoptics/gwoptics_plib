...
/**
 * <p>
 * This class encapsulates a surface trace on a graph. It handles processing each point on the grid as well as colouring it
 * it also handles relating graph space to world space(see below). This object is designed to work with the SurfaceGraph3D closely,
 * or anything that uses Axis3D objects in a Cartesian form.
 * </p>
 * <p>
 * Firstly world space is the size of everything render in the sketch, so if we specify something has a length 100 its a 100 
 * units long and at whatever position we set it at in units. Graph space on the other hand relates to the fact we can have axis 
 * ranges, for example -10 to 10, which is displayed along out axis which has a world length of 100. therefore 0 in graph space
 * relates to 50 units along the axis in world space.
 * </p>
...
public class SurfaceTrace3D extends Renderable {
	...
	//Getters
	public float getZValue(int X, int Y) {return _grid.getZValue(X, Y);}
	public float[] getMaximumPoint(){return _highestValue;}
	public float[] getLowestPoint(){return _lowestValue;}
	
	/**
	 * To generate a surface trace all 3 cartesian axis that the trace is mapped onto need to be supplied, also the 
	 * resolution of the grid in both X and Y directions.  A callback must be specified or a NullReferenceException will
	...
	public SurfaceTrace3D(PApplet parent, Axis3D x, Axis3D y, Axis3D z, int Xresolution, int Yresolution, IGraph3DCallback cb, String n, IColourmap map){
		...
		if(cb == null){throw new NullPointerException("A callback object must be specified for a trace.");}
		
		int _gridXResolution;
		int _gridYResolution;
		
		if(Xresolution < 1) {_gridXResolution = 1;}else {_gridXResolution = Xresolution;}
		if(Yresolution < 1) {_gridYResolution = 1;}else {_gridYResolution = Yresolution;}
	...
	/**
	 * This function must be called when an axis changes its range as the graph to world space constants will
	 * change.
	 */
