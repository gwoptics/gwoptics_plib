 ...
 * Gaussmode objects store the beam parameter, the laser wavelength and the local 
 * index of refraction, which fully decribes a Gaussian beam. The class provides a set
 * of utility functions to transform Gaussian beam parameters or to compute 
 * properties of the beam (beam size, amplitue patterns, etc.)
 * 
...
public class GaussMode {
	...
	/* for Hermite-Gauss modes the beam parameter in x and y 
	...
	/** 
	 * This function computes a Gaussian beam parameters from user defined parameters
	 * for the beam waist (and wavelength, and index of refraction).
	 * 
	...
	public Double w_size(Complex q) {
		assert (q.imag() > 0.0);
		return (Complex.modulus(q) / Math.sqrt(Math.PI * q.imag()) * Math
				.sqrt(_lambda / _nr));
	}
	 ...
	 * shape u_nm. This is not simply abs(u_nm) or real(u_nm) but the
	 * amplitude including the characteristic sign flips.
	 * 
	 * @param n horizontal mode index
	 * @param m vertical mode index
	 * @param x position on x-xaxis (orthogonal to beam direction) in meters
	...
	/** 
	 * This function returns the phase of a Hermite-Gaussian beam
	 * shape u_nm, *without* the sign flips from the H polynomials!
	 * 
	 * @param n horizontal mode index
	 * @param m vertical mode index
	 * @param x position on x-xaxis (orthogonal to beam direction) in meters
	...
	/**
	 * This function represents the one-dimensional part of u_nm_amp the amplitude of 
	...
	/**
	 * This function represents the one-dimensional part of u_nm_phs the phase of a 
	 * u_nm Hermite-Gauss beam pattern.
	 * @param n mode index
	 * @param q Gaussian beam parameter
	 * @param x position on axis
	 * @return complex amplitude
	...
	public Complex u_n(int n, Complex q, double x) {
		...
		z2 = Complex.sqrt(Complex.divide(Complex.multiply(q0(q), Complex
				.conj(q)), Complex.multiply(Complex.conj(q0(q)), q)));
	...
	/** 
	 * This function returns the complex amplitude of a helical 
	 ...
	 * Laguerre-Gaussian beam shape u_pl. This is not simply abs(u_pl) 
	 * or real(u_pl) but the amplitude including the characteristic sign flips.
	 * 
	 * @param p radial mode index
	 * @param l azimuthal mode index
	 * @param r radial position (orthogonal to beam direction) in meters
	 ...
	 * Laguerre-Gaussian beam shape u_pl. This is a wrapper function
	 * for u_pl_cos which uses cylindrical coordinates.
	 * 
	 * @param p radial mode index
	 * @param l azimuthal mode index
	 * @param x position on x-xaxis (orthogonal to beam direction) in meters
	...
	/** 
	 * This function returns the real amplitude only of a sinusoidal Laguerre-Gaussian 
	 * beam shape u_pl. This is a wrapper for u_pl_cos_amp which uses cylindrical 
	 * coordinates.
	 * 
	 * @param p radial mode index
	 * @param l azimuthal mode index
